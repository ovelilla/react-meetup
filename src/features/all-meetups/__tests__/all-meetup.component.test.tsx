...
import { AllMeetupsPage } from "@/features/all-meetups/all-meetups.page";
...
jest.mock("../hooks/all-meetups.hook");
...
describe("AllMeetupsPage", () => {
  ...
  it("should render MeetupListSkeletonComponent when loading is true", () => {
    (AllMeetupsHook as jest.Mock).mockReturnValueOnce({
      handleAddToFavorites: mockHandleAddToFavorites,
      handleRemoveFromFavorites: mockHandleRemoveFromFavorites,
      loading: true,
      meetups: [],
    ...
    expect(MeetupListComponent).not.toHaveBeenCalled();
  });
